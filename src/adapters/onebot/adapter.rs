...
impl Adapter for OneBotAdapter {
    ...
    fn send_message(
        ...
        let (message_type, target_id) = if message.recipient.starts_with("group_") {
            let gid: i64 = message.recipient
                .strip_prefix("group_")
                .and_then(|s| s.parse().ok())
                .unwrap_or(0);
            ("group", gid)
        } else if message.recipient.starts_with("private_") {
            let uid: i64 = message.recipient
        ...
        let action = protocol::build_send_msg_action(
            message_type,
            target_id,
            &segments,
            Some(&message.id),
        ...
        let sender = self.sender.lock().map_err(|e| {
            format!("OneBot sender mutex poisoned: {}", e)
        })?;
...
fn internal_message_to_segments(message: &Message) -> Vec<MessageSegment> {
    match message.message_type {
        ...
        crate::models::message::MessageType::File => {
            vec![MessageSegment::text(&format!("[File: {}]", message.content))]
        }
    }
}
